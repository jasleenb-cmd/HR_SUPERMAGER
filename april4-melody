...
def delete_employee():
    global employee_data, payroll_tree
    selected_employee_index = payroll_tree.focus()
    selected_employee_index = selected_employee_index[1:4]
    #print(employee_data)
    #print(selected_employee_index[1:4], type(selected_employee_index))

    if selected_employee_index:
        new_list = []
        for row in employee_data:
            #print(row[0], selected_employee_index)
            if row[0] != selected_employee_index:
                #employee_data.remove(row)
                new_list.append(row)
                #print("hi")
                #employee_data.pop(row)
            else:
...
def update_employee_list():
    #global employee_list, payroll_tree
    global payroll_tree, employee_code
    ...
    for employee in employee_data:
        ...
        if employee[0] not in employee_code:
            ...
            payroll_tree.insert("", "end", values=(employee[0], employee[1], employee[2], employee[3], ""))
...
def update_schedule_list():
    ...
    for schedule in employee_schedules:
        schedule_list.insert('end', f"{schedule[0]} - {schedule[1]} - {schedule[2]} - {schedule[3]}")
...
def create_dashboard():
    global entry_employee_id, entry_employee_name, combobox_employee_type, entry_employee_salary, employee_list, entry_event_name, entry_event_date, entry_event_time, event_list, combobox_employee_id, schedule_list, payroll_tree
    ...
    payroll_tree = ttk.Treeview(payroll_frame, columns=("eid", "employee_name", "contract_type", "unit_pay"), show="headings", height=15)
    ...
    ttk.Label(training_frame, text="Employee ID:").grid(row=0, column=0,
                                                        sticky="e")
    combobox_employee_id = ttk.Combobox(training_frame,
                                        values=[emp[0] for emp in
                                                employee_data],
                                        state="readonly")
    combobox_employee_id.grid(row=0, column=1)
    ttk.Label(training_frame, text="Event Name:").grid(row=1, column=0,
                                                       sticky="e")
    combobox_event_name = ttk.Combobox(training_frame,
                                       values=[event[0] for event in
                                               scheduled_events],
                                       state="readonly")
    combobox_event_name.grid(row=1, column=1)

    ttk.Label(training_frame, text="Event Date (YYYY-MM-DD):").grid(row=2,
                                                                    column=0,
                                                                    sticky="e")
    entry_schedule_date = tk.Entry(training_frame)
    entry_schedule_date.grid(row=2, column=1)

    button_add_schedule = tk.Button(training_frame, text="Add Schedule",
                                    command=add_schedule)
    button_add_schedule.grid(row=3, column=0, columnspan=2, pady=10)

    schedule_list = tk.Listbox(training_frame, height=10, width=50)
    schedule_list.grid(row=4, column=0, columnspan=2)
